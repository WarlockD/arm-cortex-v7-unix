...
int readi (struct inode *ip, char *dst, uint32_t off, uint32_t n)
{
    ...
    if (S_ISBLK(ip->type)) {
    	struct	bdevsw* drv = &bdevsw[major(ip->dev)];
    	if(!drv->d_read) return -1;
    	return drv->d_read(ip->dev,dst,off,n);
    }
    if (S_ISCHR(ip->type)) return -1;
...
int writei (struct inode *ip, char *src, uint32_t off, uint32_t n)
{
    ...
    if (S_ISBLK(ip->type)) {
    	struct	bdevsw* blkdev = &bdevsw[major(ip->dev)];
    	if(!blkdev->d_write) return -1;
    	return blkdev->d_write(ip->dev,src,off,n);
    }
    if (S_ISCHR(ip->type)) return -1;
