...
struct kmem {
    ...
    uint32_t            start;             // start of memory for marks
    uint32_t            start_heap;        // start of allocatable memory
    uint32_t            end;
...
{
    return ((uint32_t)mem - kmem.start_heap) >> order;
}

static inline int available (uint32_t bitmap, int blk_id)
{
...
void kmem_init2(void *vstart, void *vend)
{
    ...
    kmem.start = (uint32_t)vstart;
    kmem.end   = (uint32_t)vend;
...
{
    if ((order > MAX_ORD) || (order < MIN_ORD) || (uint32_t)mem & ((1<<order) -1)) {
