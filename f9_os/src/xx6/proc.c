...
#include "sysfile.h"
#include <os\arch.h>
#define SLEEP_SINGLE_THREAD_TEST
#include <assert.h>
...
static struct proc* allocproc(void)
{
    ...
    if((p->kstack = alloc_page ()) == 0){
        p->state = UNUSED;
        panic("allocproc: Could not alloc page!\r\n");
        return 0;
    }
...
extern void xv6_init(); /// the init routing for the os
//PAGEBREAK: 32
...
struct proc * userinit(void)
{
  ...
  //  extern char _binary_initcode_start[], _binary_initcode_size[];
    ...
    p->tf->pc = (uint32_t)error_init;
   // p->tf->spsr = spsr_usr ();
    p->tf->sp = align_up (__get_MSP(), PTE_SZ);	// set the user stack from msp
    p->tf->lr = (uint32_t)userinit; // its going to panic so it dosn't matter, but here for a stack trace

    // set the user pc. The actual pc loaded into r15_usr is in
    // p->tf, the trapframe.
    p->tf->pc = (uint32_t)xv6_init;					// beginning of initcode.S
...
#define DIFF(a,b) \
  ({ __typeof__ (a) _a = (a); \
      __typeof__ (b) _b = (b); \
    _a > _b ? _a- _b : _b-_a; })
off_t write_maps(struct inode* ip, off_t off, struct map* map, size_t count) {
	static struct map zero_map = { 0, 0 };
	while(count-- && map->addr) {
		writei(ip, (void*)map, off, sizeof(*map));
		off+=  sizeof(*map);
		if(map->size > 0) {
			writei(ip, map->addr, off, map->size);
			off+=map->size;
		}
		map++;
	}
	writei(ip, (void*)&zero_map, off, sizeof(map));
	off+=sizeof(map);
	return off;
}
off_t read_maps(struct inode* ip, off_t off) {
	struct map map;
	for(;;) {
		readi(ip, (void*)&map, off, sizeof(map));
		off+=  sizeof(map);
		if(map.size == 0 && map.addr == 0) break; // we are done
		if(map.size > 0) {
			readi(ip, map.addr, off, map.size);
			off+=map.size;
		}
	}
	return off;
}
void swapout_process(struct proc* p) {
	assert(p->state != SWAPPED);
	struct inode* ip;
	begin_trans();
	if(p->swap == 0) {
		ip = ialloc(ROOTDEV,_IFREG);
		p->swap = ip->inum;
		ip->nlink = 2;// make very sure we don't get deleted
		iupdate(ip);
	} else {
		ip = iget (ROOTDEV, p->swap);
	}
	struct map maps[2];
	maps[0].addr = (caddr_t)((p->context) + 1);
	maps[0].size = DIFF(maps[0].addr,cpu->user_stack);
	maps[1].addr = (caddr_t)p->heap_end;
	maps[1].size = DIFF(maps[1].addr,cpu->user_heap);
	write_maps(ip,0,maps,2);
	p->state = SWAPPED;
	iupdate(ip);
	iput(ip);
	commit_trans();
...
void swapin_process(struct proc* p) {
	assert(p->state == SWAPPED);
	struct inode* ip;
	begin_trans();
	if(p->swap == 0) {
...
void swap_process(struct proc* p, mode_t mode) {
	if(mode == _FREAD) swapin_process(p);
	else swapout_process(p);
}
...
#if 1
    swapout_process(np);   // copy the process to swap
#else
...
int xv6_wait(void)
{
    ...
    for(;;){
        ...
        xv6_sleep(proc, &ptable.lock);  //DOC: wait-sleep
    }
...
void xv6_sleep(void *chan, struct spinlock *lk)
{
   ...
   // }
    ...
    proc->chan = chan;
    proc->state = SLEEPING;
#ifdef SLEEP_SINGLE_THREAD_TEST
